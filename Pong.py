...
#center this label
#lbl1 = Label(window, text="PONG!", bg="orange red", fg="white", font="none 24 bold")
...
#ttk.Button(mainframe, text="PLAY", command=main).grid(column=4,row=8,sticky=E)

#ttk.Button(mainframe, text="Quit", command=quit).grid(column=4,row=8,sticky=E)
...
pygame.mixer.music.load('WorkingGame.mp3')
pygame.mixer.music.play(-1)
#setting the FPS or number of Frames per Second
FPS = 60

#Setting the screen size
scr_size = (width,height) = (700,500)
...
pygame.display.set_caption('PONG by Enersom')
font = pygame.font.Font('freesansbold.ttf', 32) 
text = font.render('Welcome to PONG Enersom', True, green, blue) 
"""
...
def cpumove(cpu,ball):
    if ball.movement[0] > 0: #ensures that the CPU moves only when the ball is directed towards it
        ...
        if ball.rect.bottom > cpu.rect.bottom + cpu.rect.height/5:
            cpu.movement[1] = 8
            #click = pygame.mixer.Sound("C:\\Users\\User\\OneDrive\\Documents\\Sound recordings\\Click.wav")
    ...
    while not gameOver: #running our game loop
        ...
        if pygame.sprite.collide_mask(cpu,ball):
            ...
            if ball.movement[1] > ball.maxspeed:
                ball.movement[1] = ball.maxspeed
                playsound("LowClick.m4a")
                cpuclick = pygame.mixer.Sound("Click.wav")
            if ball.movement[1] < -1*ball.maxspeed:
                playsound("LowClick.m4a")
                ball.movement[1] = -1*ball.maxspeed
                playsound("LowClick.m4a")
